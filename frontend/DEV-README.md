...
```bash
bash EADAF_frontend/shells/refresh-ai-base.sh
```
...
1. 输出到浏览器控制台
2. 转发到 **EADAF_frontend dev 终端**（`pnpm dev` 窗口）

Server Tool 日志输出到 **EADAF_backend 终端**。
...
```bash
# 确保 EADAF_backend 已启动且 Swagger 可访问
cd EADAF_frontend
pnpm openapi2ts
```
