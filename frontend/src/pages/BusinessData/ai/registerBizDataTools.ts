...
import { getApiData, getApiErrorMessage, parseApiListResponse } from '@/utils/apiResponse';
import {
  loadEntity,
  loadEntityFields,
  mergeEntityFields,
...
export function registerBizDataTools() {
  ...
  registerFunctionCall({
    ...
    description:
      '创建实体；可同时传入 fields、indexes、relations，一次性完成字段、索引与关系建模（推荐完整建模时使用）',
    parameters: {
      type: 'object',
      properties: {
  ...
  registerFunctionCall({
    ...
    description: '更新实体元信息、字段、layout 或 jsonSchema（字段默认与已有合并，replaceFields=true 时全量替换）',
    parameters: {
      type: 'object',
      properties: {
    ...
    handler: createMutatingHandler({
      ...
      handler: async (args) => {
        ...
        if (Object.keys(patchPayload).length) {
          await patchBusinessDataEntity(entityId, patchPayload);
        }
  ...
  registerFunctionCall({
    ...
    handler: createMutatingHandler({
      ...
      type: 'entity.deleted',
      scope: BIZDATA_SURFACE,
      buildResourceId: (args) => String(args.entityId),
      buildPayload: () => ({ success: true }),
      handler: async (args) => {
        await deleteBusinessDataEntity(String(args.entityId));
        return { success: true, entityId: String(args.entityId) };
      },
    }),
  });
