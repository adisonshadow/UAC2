/**
 * AI Tool 执行日志：输出到 EADAF_backend 终端
 */
...
function formatToolInvokeError(error) {
  if (!error) return 'unknown error';
  const formatted = formatApiError(error);
  let text = formatted.status ? `[HTTP ${formatted.status}] ${formatted.message}` : formatted.message;
  const detail = stringifyDetail(formatted.data);
