...
const { isValidSlug } = require('../constants/aiCapabilities');
...
class ScopeController {
  ...
  static async create(ctx) {
    ...
    if (!trimmedSlug) {
      ctx.status = 400;
      ctx.body = { code: 400, message: 'Scope ID 为必填项', data: null };
      return;
    }
    if (!isValidSlug(trimmedSlug)) {
      ctx.status = 400;
      ctx.body = { code: 400, message: 'Scope ID 格式无效', data: null };
      return;
    }
    ...
    try {
      ...
      if (existing) {
        ctx.status = 400;
        ctx.body = { code: 400, message: 'Scope ID 已存在', data: null };
        return;
      }
