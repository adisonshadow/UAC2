...
const schemas = {
  ...
  OperationLog: {
    ...
    properties: {
      ...
      operatorType: { type: 'string', enum: ['USER', 'APPLICATION', 'SYSTEM', 'ANONYMOUS'] },
      applicationId: { type: 'string', format: 'uuid', nullable: true },
      domain: { type: 'string', nullable: true },
      operationType: str('操作类型', 'UPDATE'),
      resourceType: str('资源类型', 'user'),
      ...
      requestSummary: { type: 'object', nullable: true },
      createdAt: dateTime('创建时间'),
    },
  },
  ...
  BizdataMaterializationStatusItem: {
    ...
    properties: {
      entityId: uuid('实体 ID'),
      code: str('编码'),
      entityCode: str('实体编码（与 code 相同，便于 AI/脚本按 entityCode 过滤）'),
      label: str('显示名'),
      tableName: str('表名'),
...
Object.assign(schemas, {
  ...
  EnvelopeOperationLog: envelope({ $ref: '#/components/schemas/OperationLog' }, { message: '获取操作日志详情成功' }),
  EnvelopeOperationLogList: envelope(paged('#/components/schemas/OperationLog'), { message: '获取操作日志成功' }),
  ...
  EnvelopeAppTransferPreview: envelope(
    obj('应用导出文件预览摘要', {
      ...
      connectionMatches: { type: 'array', items: { type: 'object' }, description: '连接匹配结果(matched / willCreate / unmatched;willCreate=导入时用目标同类型凭证创建本地连接)' },
      physicalTables: { type: 'array', items: { type: 'object' }, description: '物化库表摘要(表名/schema/列数/行数)' },
